...
@section('content')
    <div class="w-full card-portal py-8 text-center">
        ...
        @if($territory->deleted_at != null && $territory->deleted_at <= Illuminate\Support\Carbon::now()->subdays(14))
            <p class="block mx-auto text-3xl">Deleted {{ Illuminate\Support\Carbon::make($territory->deleted_at)->diffForHumans() }}, not eligible for modification </p>
        @else
        ...
        <p class="block mx-auto">
            <a type="button"
               class="inline-block px-6 py-2.5 btn-portal font-medium text-xs leading-tight uppercase rounded shadow-md transition duration-150 ease-in-out"
               href="{{ route('logs.list', ['startDate' => \Carbon\Carbon::now()->subDays(3)->toDateString(), 'endDate' => \Carbon\Carbon::now()->toDateString(), 'searchString' => $territory->id, 'searchColumn' => 'territory_id']) }}">
                Show Logs
            </a>
        </p>
    </div>
