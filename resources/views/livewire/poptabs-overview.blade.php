<div>
    <div class="container mx-auto px-4 sm:px-8 container-portal mt-10">
        <div class="py-8">
            <div>
                <h2 class="text-2xl font-semibold leading-tight"> {{ config('portal.instanceName') }}'s Forbes Magazine</h2>
            </div>
            ...
            <div class="-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
                <div class="min-w-full shadow rounded-lg overflow-hidden text-portal-red">
                    {{ $accounts->links() }}
                        ...
                        @foreach($accounts as $account)
                            <tr class="table-row-portal">
                                <td class="px-5 py-5 text-center text-sm">
                                    <a class="whitespace-no-wrap underline" href="{{ route('account.view', ['account' => $account->uid]) }}">{{ $account->name }}</a>
                                </td>
                                <td class="px-5 py-5 text-center text-sm">
                                    <a class="whitespace-no-wrap underline" href="{{ route('account.view', ['account' => $account->uid]) }}">{{ $account->uid }}</a>
                                </td>
                                <td class="px-5 py-5 text-center text-lg">
                                    <p class="whitespace-no-wrap">{{ number_format($account->container_sum_money) }}</p>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
