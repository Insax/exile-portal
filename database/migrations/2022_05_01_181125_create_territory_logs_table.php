...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('territory_logs', function (Blueprint $table) {
            ...
            $table->integer('poptabs_before');
            $table->integer('poptabs_after');
            $table->string('player_pos');
            $table->string('target_account_uid', 17)->nullable();
            $table->timestamp('time');

            $table->foreign('account_uid')->references('uid')->on('accounts')->onDelete('cascade');
            $table->foreign('target_account_uid')->references('uid')->on('accounts')->onDelete('cascade');
        });
    }
    ...
    {
        Schema::dropIfExists('territory_logs');
    }
};
