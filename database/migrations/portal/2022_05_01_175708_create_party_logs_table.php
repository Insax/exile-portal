...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('party_logs', function (Blueprint $table) {
            ...
            $table->string('invited_account_uid', 17);
            $table->foreignId('invited_player_clan_id')->nullable()->constrained('clans')->onDelete('cascade');
            $table->text('group_name');
            $table->timestamp('time');

            $table->foreign('account_uid')->references('uid')->on('accounts')->onDelete('cascade');
            $table->foreign('invited_account_uid')->references('uid')->on('accounts')->onDelete('cascade');
        });
    }
